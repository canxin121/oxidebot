...
use oxidebot_core::{
    BotId, EventId, MessageContent, MessageCreated, MessageOptions, OutgoingMessage, PlatformId,
};
use oxidebot_runtime::{
    message, on, AskOptions, Context, Outcome, OverloadPolicy, OxideBot, QueueBudget,
    RuntimeConfig, RuntimeMetrics, RuntimeProfile, Service, ServiceContext, ServiceError,
};
...
#[tokio::test]
async fn only_indexed_handler_creates_a_future() {
    ...
    let mut app = OxideBot::new().bot(adapter);
    for index in 0..100 {
...
async fn exact_session_consumes_response_before_normal_routes() {
    ...
    assert!(matches!(
        sent[0].message.content.as_slice(),
        [MessageContent::Text(text)] if text.as_ref() == "send a number"
    ));
    assert!(matches!(
        sent[1].message.content.as_slice(),
        [MessageContent::Text(text)] if text.as_ref() == "answer=42"
    ));
}
